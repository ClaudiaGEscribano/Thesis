...
\chapter{Multi-step scheme and spatial analysis of long-term characteristics of photovoltaic productivity over the Iberian Peninsula\label{multi}}
...
Although the AOD dataset and the climate simulations used in this study have already been evaluated against observations and satellite datasets in \cite*{Nabat2013, Nabat2015, Nabat2015a}, an additional assessment of the SSR form the climate simulations is also made in this work against the CM-SAF SARAH dataset \cite*{Muller2015} for the period 2003-2009 and against some BSRN \cite*{Konig-Langlo2013} stations at a local scale. 
...
\begin{abstract}
  ...
  In this chapter we try to explain the reasons behind the different behavior focusing on the representation of aerosols in the scenario simulations of RCMs. We use regional climate models from the EURO-CORDEX project and we focus on the RCP8.5.
  
  Regional climate models with evolving aerosols scenarios reverse the sign of the signal in the shortwave solar radiation over Europe for mid XXI century, although with differences in magnitude. Besides, it is not find a clear relationship between the SSR anomalies in regional climate models including aerosols and the anomalies in total cloud cover.
  ...
  Projected changes in SSR significantly impact the PV potential for the simulations with evolving aerosols. In summer, anomalies can reach 9$\%$ and above of relative PV increase for the ALADIN53 simulation in Central Europe. The projected increase in Germany is 10.89$\%$. In southern countries a moderate increase is projected, like 5.63$\%$ in Greece or 6.44$\5$ in Italy. An overall increase is also projected for RACMO22E with differences in the spatial pattern with the maximum in the South-Eastern countries, and smaller in the magnitude. In Italy it is projected an increase of 1.63$\%$ and 2.35$\%$ in Greece.
...
The spatial pattern of SSR anomaly is different in the two simulations. The ALADIN-RCSM4 shows a generalized increase over the domain with values around 10 $W/m^2$ and reaching 15 $W/m^2$  in certain areas. For PROTHEUS, the spatial pattern shows negative anomalies in the North of the domain. Positive values are around 5 $W/m^2$ in the Southern part of Europe with some areas of 10 $W/m^2$. South-Western area of the domain, which roughly correspond to the IP, reaches highest values of anomaly, close to 15 $W/m^2$.

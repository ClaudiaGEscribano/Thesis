...
\chapter[Multi-step scheme over the Iberian Peninsula]{Multi-step scheme and spatial analysis of long-term characteristics of photovoltaic productivity over the Iberian Peninsula\label{multi}}
...
\begin{abstract}
  ...
  In the last decades, trends in photovoltaic technology deployment show an overall increase over the world. The comprehensive knowledge of solar resource and its future evolution is demanded by the energy sector. Solar resource and photovoltaic potential have been estimated in several studies using both global (GCM) and regional climate models (RCM) revealing a GCM-RCM discrepancy even in the sign of the change over Europe. An increase in surface solar radiation (SSR) (respectively in photovoltaic potential production) is projected by GCMs, whereas most of the RCMs simulations project a decrease in SSR. In this work, we investigate the role of the aerosol forcing in RCMs as a key explaining factor of this inconsistency. The results show that RCM simulations including evolving aerosols agree with GCMs in the sign and amplitude of the SSR anomaly over Europe for mid-21st century projections (2021-2050 with respect to 1971-2000, RCP8.5). The opposite signal is projected by the rest of RCMs. The amplitude of the anomalies depends on the model. In terms of photovoltaic potential productivity, RCMs including evolving aerosols simulate an important yearly increase especially in Central and Eastern Europe with  maximum values reaching +10$\%$ for some countries in Summer (in one of the RCM). On the contrary, the RCMs with no evolving aerosols show a negative anomaly of PV production. This study illustrates the key role of the often-neglected aerosol forcing evolution in RCMs and argues for its inclusion in the next generation of the CORDEX simulations.
  ...
  % Although climate models give a robust answer in terms of global warming, they disagree in the projected changes of solar resource over Europe. Whereas global climate models, GCMs, present a clear positive signal for the end of XXI century, some studies show a negative anomaly for the same period in RCMs.
  
  % In this chapter we try to explain the reasons behind the different behavior focusing on the representation of aerosols in the scenario simulations of RCMs. We use regional climate models from the EURO-CORDEX project and we focus on the RCP8.5 emissions scenario.
  
  % Regional climate models with evolving aerosols scenarios reverse the sign of the signal in the shortwave solar radiation over Europe for mid XXI century, although with differences in magnitude. Besides, there is no clear relationship between the SSR anomalies in regional climate models including aerosols and the anomalies in total cloud cover.
  ...
  % Projected changes in SSR significantly impact the PV potential in the simulations with evolving aerosols. In summer, anomalies anomalies of relative PV increase can reach 9$\%$ and above for the ALADIN53 simulation in Central Europe. The projected increase in Germany is 10.9$\%$. In southern countries a moderate increase is projected, like 5.6$\%$ in Greece or 6.4$\5$ in Italy. An overall increase is also projected for RACMO22E with differences in the spatial pattern with the maximum in the South-Eastern countries, and smaller in the magnitude. In Italy it is projected an increase of 1.6$\%$ and 2.3$\%$ in Greece.
...
The summer (JJA) mean anomaly of the period 2021-2050 with respect to the reference period 1971-2000 is represented in figures \ref{fig:anomalySSR} and \ref{fig:anomalyCLT} first for surface solar irradiation, SSR, and then for total cloud cover, CLT.

\begin{table}[h!]
  \caption{\label{tb:spatial correlations}Spatial anomalies with respect of the reference period for SSR, CLT and AOD; and spatial correlation between SSR,CLT and AOD anomalies maps.}
\footnotesize
...
\midrule 
%\mr  
  CNRM-CM5& & 9.85 & 0.66 & &-0.38 &  \\
            &ALADIN5.3 & 13.19 & 0.29 & -0.16 & -0.22 & -0.93\\
            &CCLM4-8-17& -2.82 & -0.82 & - & -0.73 & -  \\
            &RCA4& -2.81 & 0.21 & - & -0.80 & - \\
\midrule
%\mr   
  ICHEC-EC-EARTH& &5.63 & -0.32 & &-0.35 &  \\
          &RACMO& 4.81 & 0.50 & -0.15 & -0.35 & -0.57 \\
          &CCLM4-8-17& -3.04 & -0.85 & - & -0.80 & - \\
          &RCA4&-2.29 & 0.13 & - & -0.78 & - \\
\bottomrule
%\br
\end{tabular}\\
...
\begin{table}[h!]
\caption{\label{tb:pvanomalies}Relative anomaly of yearly PV and JJA PV with respect of the reference period for the whole domain and JJA PV relative anomaly averaged by country.}
\footnotesize
...
\midrule 
%\mr   
  CNRM-\\CM5&ALADIN5.3& 5.9$\%$ & 4.8 $\%$ & 2.4$\%$ & 10.9$\%$ & 6.4$\%$ & 4.6$\%$ & 11.2$\%$ & 12.2$\%$ \\
          &CCLM4-8-17& -2.7$\%$ & -1.5$\%$ & -0.6$\%$ & -2.3 $\%$ & -0.4$\%$ & 0.3 $\%$ &-1.0$\%$ &-1.2$\%$ \\
          &RCA4& -2.8$\%$ & -1.3 $\%$ & -0.8 $\%$ & -1.5 $\%$ & -0.4 $\%$ & -0.4$\%$ &  -0.5$\%$ &-1.1$\%$ &\\
\midrule
%\mr  
  ICHEC-\\EC-EARTH &RACMO& -1.2$\%$ & 1.2$\%$ & 0.4$\%$ & 1.4$\%$ & 1.6$\%$ & 2.4$\%$ & 4.0$\%$ &2.5$\%$\\
                 &CCLM4-8-17& -4.4$\%$ & -2.0$\%$ & -0.6$\%$ & -1.9$\%$ & -0.6$\%$ & -1.4$\%$ & -1.8$\%$ &-1.8$\%$\\
                 &RCA4& -4.0$\%$ & -1.4$\%$ & -0.8$\%$ & -0.3$\%$ & -0.6$\%$ & -0.9$\%$ & -0.7$\%$ &-0.5$\%$\\
\bottomrule
%\br
\end{tabular}\\
\end{table}
...
In summer months the relative mean change for ALADIN53 is above 10$\%$ in Germany, Poland, Austria etc and maximum values between 3 and 5$\%$ are projected with RACMO22E in the East and South-East of Europe.  
...
In figure \ref{fig:medcordexclt} the CLT's change for the two models can be seen with respect to the reference period. In this case in opposition to the SSR anomaly, the spatial pattern of both models is similar, projecting a decrease of CLT for the Mediterranean area and a slight increase in Central to Northern Europe. Nevertheless, the decrease of CLT in ALADIN-RCSM4 is smaller.
...
Mean anomaly for the whole domain is larger for ALADIN-RCSM4 than for PROTHEUS, but the CLT mean anomaly is similar for both. The spatial correlation between CLT and SSR is highly negative for PROTHEUS and less negative for ALADIN-RCSM4.

\begin{table}[h!]
\caption[SSR, CLT anomalies and spatial correlation for MED-CORDEX models]\label{tb:medcordexspcor}SSR, CLT anomalies and spatial correlation between them.}
%\footnotesize
\begin{tabular*}{0.8\textwidth}{@{}llllllll}
\br
& $\rho_{SSR,CLT}$  & $\rho_{SSR,AOD}$ & $\Delta{SSR}$ [$\si{\watt\per\metro\squared}$] & $\Delta{CLT}$ $[\%]$ \\
\midrule
%  \mr 
...
Differences between anomalies in both climate models indicate that changes in SSR are not only influenced by cloudiness. In addition to the analyses developed in chapter 7, these preliminary results show that a deeper research on how aerosols influence future projections of solar resource should be done, air-sea coupling may also affect changes in SSR indirectly. The results from CORDEX initiative FPS-AEROSOLS, will help to reduce the uncertainty in shortwave solar radiation future projections, helping in the application of solar energy modeling for the future. 

...
\nomenclature[R]{$R(0)$}{Albedo component}
...
\nomenclature[BSRN]{$BSRN$}{Baseline Surface Radiation Network}
...
The aim of CM-SAF consortium from EUMETSAT is to develop satellite-data-based-products for climate monitoring since 2000 when the importance of using satellite data for this purpose was recognized \cite*{schulz2009operational}.
...
\nomenclature[CM-SAF]{$CM-SAF$}{The Satellite Application Facility on Climate Monitoring}
...
\nomenclature[RTM]{$RTM$}{Radiative Transfer Model}
...
The main difference between the two initiatives, in addition to the studied domain represented in figure \ref{fig:cordexdomain}, is that simulations from Med-CORDEX \cite*{Ruti2016} are runs from coupled ocean-atmosphere climate models, whereas the Euro-CORDEX \cite*{Jacob2014} ensemble includes atmosphere models only. %(¿?o indistintamente)

%{\color{red} Incluir como mucho la diferencia entre MED y EURO cordex así como FPS}

\section{Aerosols datasets}

Atmospheric aerosols content can be summarized by the AOD, aerosol optical depth, parameter; which is the integration among an atmospheric column of the incident light that is scattered or absorbed by aerosols. Since the 1980s, different sensors from satellites have been used to derived AOD and at the same time, some models have been developed and provide AOD climatologies as well as the contribution of different species to the total AOD \cite*{Nabat2013}.
...
%Up to now, only in [nabat2014], simulations with ALADIN-RCSM4 are able to reproduce SSR trends over Europe in past decades after applying an interannual aerosols dataset described in \cite*{Nabat2013}. A more recent study includes ammoniun and nitrates species for long simulations (197-2016) over the Euro-Mediterranean area with the same RCMs (ALADIN-RCSM) with an interactive aerosol scheme. [drughe, tactic]
...
%Information about aerosols datasets and description (prescribed, interactive schemes etc.) is not an information easy to access from the website where climate projections users can download different simulations. Instead, a task of compiling information from modelers is needed, which difficults the process. %In next table a summary of the known aerosols information of climate simulations from EURO-CORDEX is included.
...
\section{Other data}%Temperature data}
...
\nomenclature[POA]{$POA$}{Plane of the Array}
...
\nomenclature[G(alfa,beta)]{$G(\alpha, \beta)$}{Global irradiation at the plane of the array. Alpha and beta correspond to the inclination and orientation angles of the panel.}
...
The second step that calculates the energy output, provides the performance of the electrical components from the global effective irradiation and other variables that can influence on cells, like ambient temperature. The whole photovoltaic system includes the generator, the inverter and the transmission elements and wires.
...
\nomenclature[Impp]{$I_{mpp}$}{Intensity for the maximum power point from I-V curve}
\nomenclature[Vm]{$V_m$}{Voltage of a photovoltaic module}
\nomenclature[Vmpp]{$V_{mpp}$}{Voltage for the maximum power point from I-V curve}
...
\nomenclature[V_oc]{$V_{oc}$}{Open circuit voltage}
\nomenclature[I_sc]{$I_{sc}$}{Short circuit current}
...
\nomenclature[G_eff]{$G_{eff}(\alpha,\beta)$}{Global effective irradiation at the tilted panel}
...
The history of climate modeling has been linked to the computational development since its beginning. The mathematical representation of the climate system was a consequence of the advance in numerical weather prediction models that took place for the first time in Princeton in 1952 and that had a fast development \cite*{Edwards2011}. % and {\color{red}develop a bit more?}].

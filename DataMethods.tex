...
The measurement of solar radiation is based on radiometers that measure the solar energy reaching the earth's surface. These radiometers are able to measure electromagnetic solar radiation at different wavelengths. For energy purpose, the interest of solar radiation measurements is on the shortwave range of the spectrum. These kind of intruments are normally of three different types: thermopile, blackbody cavity, and solid state \cite*{Gueymard2008}. The detector has a known spectral response to incident radiation and it is protected with an optical window that can be used to limit the spectral range of the radiation measured. 

Solar radiation can be decomposed in three components, as it is explain in section []. Different instrumentation can be used to measure each of the components. For global solar irradiation pyranometers can be used whereas for direct solar irradiation, the instruments are called pyrheliometers.
...
First intruments that measures solar radiation were based on intruments to measure the duration of daytime.
...
From the measurements side, the \tectbf{AERONET} network \cite*{Holben1198} was initiated by NASA in the 90s and it has grown to become a widely spread network of radiometers accross different continents based on international cooperation. This ground-based aerosols monitoring allows to characterize from local to global profiles directly related with earth's radiation budget studies. It has become also an important database for evaluation of satellite derived and model AOD products.

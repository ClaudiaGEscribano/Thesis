...
Main difference between the two initiatives, in addition to the studied domain represented in figure \ref{fig:cordexdomain}, is that simulations from MED-CORDEX are runs from coupled ocean-atmosphere climate models, whereas the EURO-CORDEX ensemble inludes only atmosphere models. %(¿?o indistintamente)
...
There are different datasets for the Mediterranean area, very influenced by natural and antrophogenic aerosols derived from satellite observations, like MODIS, MISR, or SEVIRI; or from model simulations like Reg-CM4 or MACC reanalysis. The first intercomparison of aerosols datasets can be found in \cite*{Nabat2013} where an improved 4-D climatoloy of aerosols is developed for the area.

\subsection{Aerosols in RCMs}

{\color{red} reescribir con la info del paper de pierre}
